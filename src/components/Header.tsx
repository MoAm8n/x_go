...
const LanguageDropdown = () => {
  ...
  const handleSelect = (lang) => {
    ...
    setOpen(false);
    // هنا تقدر تضيف منطق تغيير اللغة في التطبيق لو عندك i18n
  };
  ...
  return (
    <div className="relative inline-block">
      <button
        ...
        className="px-4 py-2 rounded-md font-medium text-[14px]  flex items-center gap-2"
        type="button"
      >
        {selected}
        <svg
      ...
      {open && (
        <ul className="absolute left-0 top-10 bottom-full mb-2 bg-white border  w-40 z-50">
          {languages.map((lang) => (
            <li
              key={lang.code}
                    ...
                    <HashLink
                      ...
                      className={
                        isHashActive(link.path)
                          ? "text-[#E6911E] pb-1 border-b-2 border-[#E6911E] font-semibold"
