...
<footer>
    ...
    <div className="max-md:w-full flex flex-col items-center md:items-start mb-6 md:mb-0 md:mr-20">
        ...
        <p className="text-center text-black text-lg font-normal md:text-left max-w-xs">{t("Stay Connected with us to get new update information")}</p>
    </div>
    <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-4 w-full text-center md:text-left">
        {FooterLinks.map((section) => (
            <div key={section.id} className="flex flex-col">
                <h3 className="font-normal text-base text-[#7B7B7B] mb-2">{t(section.title)}</h3>
    ...
    <div className="text-center text-gray-400 text-xs mt-8 mb-8">&copy; {new Date().getFullYear()} {t("Car App. All rights reserved.")}</div>
</footer>
)};
