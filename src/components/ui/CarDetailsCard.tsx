...
useEffect(() => {
    ...
    try {
        ...
        const filteredCars = fetchedCars.filter(car =>
            (filters.selectedBrands.length === 0 || (car.brandId && filters.selectedBrands.includes(car.brandId))) &&
            (filters.selectedTypes.length === 0 ||
                (car.type && filters.selectedTypes.some(
                    t => t.toLowerCase() === car.type?.toLowerCase()
                ))
            ) &&
            (typeof car.price === 'number' && car.price >= filters.priceRange[0] && car.price <= filters.priceRange[1])
        );
