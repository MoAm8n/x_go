    ...
    <div className="bg-white rounded-xl shadow-md overflow-hidden">
      <div className="p-6">
        <button
          ...
          aria-label="العودة إلى قائمة الحجوزات"
        >
          <svg
        ...
        <h1 className="text-2xl font-bold mb-6">تفاصيل الحجز #{booking.id}</h1>
              ...
              <div className="flex flex-col md:flex-row gap-6">
                <div className="md:w-1/3">
                  <img
                    ...
                    alt={`${booking.car_model?.relationship?.Brand?.brand_name || 'غير معروف'} ${
                      booking.car_model?.relationship?.Types?.type_name || "غير معرف"
                    }`}
                ...
                <div className="md:w-2/3">
                  <h2 className="text-xl font-bold">
                    {booking.car_model?.relationship?.Brand?.brand_name || 'غير معروف'}{' '}
                    {booking.car_model?.relationship?.Types?.type_name || 'غير معروف'}
                  </h2>
                  <p className="text-gray-500">{booking.car_model?.attributes?.year || 'غير معروف'}</p>
                  ...
                  <div className="flex flex-wrap gap-2 my-4">
                    ...
                    <div className="flex items-center gap-1 px-3 py-1 border rounded-full">
                      ...
                      <span>{booking.car_model.attributes?.seat_type || '4'} مقاعد</span>
                    </div>
                    <div className="flex items-center gap-1 px-3 py-1 border rounded-full">
                  ...
                  <div className="grid grid-cols-2 gap-4 mt-4">
                    <div>
                      <p className="text-gray-500">نوع المقاعد</p>
                      <p>{booking.car_model?.attributes?.seat_type || 'غير معروف'}</p>
                    </div>
                    <div>
                      <p className="text-gray-500">التسارع (0-100 كم/س)</p>
                      <p>{booking.car_model?.attributes?.acceleration || 'غير معروف'} ثانية</p>
                    </div>
                    <div>
                      <p className="text-gray-500">نوع السيارة</p>
                      <p>{booking.car_model?.relationship?.Types?.type_name || 'غير معروف'}</p>
                    </div>
                    <div>
                      <p className="text-gray-500">السعر اليومي</p>
                      <p>${booking.car_model?.attributes?.price || 'غير معروف'}</p>
                    </div>
                  </div>
                </div>
              </div>
