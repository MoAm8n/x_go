...
const AddCarBrand = ({ carBrand, setCarBrand, carLogo, setCarLogo }: AddCarBrandProps) => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .catch((err) => {
        console.error(t("add_car_brand.error_fetching_brands"), err);
        toast.error(t("add_car_brand.error_fetching_brands"));
      });
  }, [t]);
    ...
    if (newBrandName) {
      ...
      axios
        ...
        .then((res) => {
          ...
          toast.success(t("add_car_brand.brand_added"));
        })
        .catch((err) => {
          const errorMsg = err.response?.data?.message || t("add_car_brand.error_adding_brand");
          console.error(t("add_car_brand.error_adding_brand_log"), err);
          toast.error(errorMsg);
        });
    } else {
      toast.warning(t("add_car_brand.fill_brand_name"));
    }
  };

  return (
    <div className="flex flex-col gap-4">
      <label className="font-semibold mb-1">{t("add_car_brand.car_brand")}</label>
      <select
      ...
      >
        <option value="">{t("add_car_brand.select_brand")}</option>
        {brands.map((brand) => (
      ...
      >
        {t("add_car_brand.add_new_brand")}
      </button>
      {showAddBrand && (
        <div className="bg-[#FAF7F2] border border-[#E6911E] rounded-xl shadow p-4 flex flex-col gap-3 mt-2">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
            <div className="flex-1">
              <label className="block mb-2 font-medium">{t("add_car_brand.add_car_brand")}</label>
              <input
                type="text"
                className="border border-[#E6911E] rounded-xl px-3 py-2 w-full bg-[#FAF7F2] focus:outline-none focus:ring-2 focus:ring-[#E6911E]"
                placeholder={t("add_car_brand.car_brand_placeholder")}
                value={newBrandName}
                onChange={(e) => setNewBrandName(e.target.value)}
              />
            </div>
            <div className="flex flex-col items-center">
              <label className="block mb-2 font-medium">{t("add_car_brand.add_car_logo")}</label>
              <label className="w-20 h-20 flex items-center justify-center border-2 border-dashed rounded-xl cursor-pointer bg-[#FAF7F2]">
                ...
                {newBrandLogo ? (
                  <img
                    ...
                    alt={t("add_car_brand.car_logo_alt")}
          ...
          >
            {t("add_car_brand.save")}
          </button>
        </div>
      )}
      {carLogo && (
        <div className="mt-2 flex items-center gap-2">
          <span className="text-xs text-gray-500">{t("add_car_brand.selected_logo")}</span>
          <img
